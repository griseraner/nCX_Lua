...
OnShutdown = function()
	System.LogAlways("SYSTEM ONSHUTDOWN");
...
if (CVars) then
	...
	for CVar in CVars:lines() do
		...
		if (not nCX.Config.CVars[CVar] and not nCX.Config.KeepSynching[CVar]) then -- and foundSV) then
			nCX.SetCVarSynch(CVar, false);
			us = us + 1;
		end
	end
	if (us > 0) then
		System.LogAlways("[nCX] Unsynched "..us.." CVars.");
	end
	CVars:close();
end
...
function nCX:Bootstrap(reload)
	...
	if (not CryMP or (CryMP and reload)) then
		...
		if (not a) then
			System.LogAlways("[nCX Lua] Aborted bootstrap (Core loading failed: "..b..").");
			return;
		end
